...
> **v1.2 변경점**: WORKBOOK 미션1의 기준 질문 **Q1~Q5가 의미 있는 분석 신호**를 갖도록 테이블 간 키 매핑과 값 분포를 재구성했습니다(예: 배송 지연↔반품/불만, 품절↔취소/문의, 캠페인 결제실패↔전환율, 프로모션 유형↔마진/재구매율, 고객등급↔재구매율). 의도적 노이즈(참조무결성/중복/결측/이상값/비표준코드셋/금액정합)와 명명 상품 시나리오는 **그대로 보존**됩니다. 데이터는 [`generate_track1_samples.py`](generate_track1_samples.py)로 재현 가능합니다(seed 고정).
...
> 데이터 재생성이 필요하면 `python generate_track1_samples.py`를 실행하세요. 스크립트는 seed가 고정되어 있어 위 신호와 모든 의도적 노이즈를 동일하게 재현합니다.
