...
## 한 줄 메시지
Track2는 Track1에서 준비된 구조를 실제 업무 근거 데이터로 확장해, 품질 게이트(8개 중 6개 PASS)와 배포 재현성을 검증하고 Track3 WebIQ 및 Track4 FoundryIQ가 사용할 WorkIQ 근거 소스를 안정화한다.
...
PASS >= 75]
    S --> M[Manifest 생성
근거 메타데이터]
    M --> T3[Track3 WebIQ 공개 확인의 내부 맥락]
    M --> T4[Track4 FoundryIQ WorkIQ 입력]
```
...
| manifest 누락/불일치 | Track4 FoundryIQ 인계 차단 + 재생성 | 인계 차단 경고 | BLOCKED |
...
5. **Track4 연결 보장**: WorkIQ 검색 가능한 구조와 키워드 일치성 검증
