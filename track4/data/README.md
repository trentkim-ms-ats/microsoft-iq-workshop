# Track4 FoundryIQ 실행 패키지

`track4/data/` is the canonical FoundryIQ runtime location. It generates and evaluates
the combined response while `track3/data/` remains the canonical WebIQ fixture location.

## Files and compatibility

| File | Purpose |
| --- | --- |
| `generate_track3_samples.py` | Track1 CSV + Track2 manifest input generation |
| `run_track3_simulation.py` | compatibility regression harness |
| `evaluate_track3_outputs.py` | compatibility strict evaluator |
| `run_microsoft_iq_simulation.py` | FabricIQ + WorkIQ + Track3 WebIQ + FoundryIQ simulation |
| `evaluate_microsoft_iq_outputs.py` | Microsoft IQ citation/fallback strict evaluator |
| `foundry_responses.py` | Azure AI Foundry Responses API helper |
| `Track3_*Notebook.ipynb` | legacy-named Foundry notebooks |

The `track3` file names and `TRACK3_*` notebook variables are legacy Track3 FoundryIQ
identifiers retained for compatibility. Their default working root and generated-output
contract are `track4/data/`.
...
track4/data/generated/
  ...
  track3_seed_summary.json       # retained legacy basename
  responses/                     # compatibility harness output
  microsoft_iq_responses/        # Microsoft IQ flow output
  reports/
```

Generated artifacts under `generated/` should be treated as run evidence. Keep
path interpretation aligned with this README and current track contracts.
...
`run_microsoft_iq_simulation.py` defaults to
`../../track3/data/web_evidence_fixture.json`; `evaluate_microsoft_iq_outputs.py` defaults to
`../../track3/data/source_catalog.json`. Both fail explicitly if those inputs are absent.

## Modes and fallback

| Mode | Sources | Meaning |
| --- | --- | --- |
| `simulation` | Track1 CSV, Track2 manifest, Track3 fixture | offline education and regression |
| `live` | FabricIQ/WorkIQ workshop adapters, approved Web Search or WebIQ adapter | separately approved connection validation |

The local Microsoft IQ workshop scripts deliberately produce `executionMode=simulation`. They do not
call an endpoint. The fixed retry policy is 5/10/20 seconds and 3 retries after the
initial call. Fabric, Work, or Web failures are partial with their explicit warnings;
both internal failures are blocked even when WebIQ succeeds.

## live adapter and Responses API contract

Adapters receive `scenarioId`, `question`, and `semanticKeys`. They must return their
own named source fields and `sourceTrace`; adapter, ACL, permission, schema, and HTTP
errors must remain explicit failures.

The optional final briefing helper uses only:

- `AZURE_AI_FOUNDRY_RESPONSES_ENDPOINT`
- `AZURE_AI_FOUNDRY_MODEL`
- `AZURE_AI_FOUNDRY_API_KEY` or `AZURE_AI_FOUNDRY_BEARER_TOKEN`

It sends API keys with `api-key` and Entra tokens with `Authorization: Bearer`. Missing
configuration skips optional finalization; it does not fabricate a success response.

## Daily reference helper

`tools/run_track3_daily_briefing.py` and the similarly named workflow/template files
are retained legacy helper names. They operate on `track4/data/generated/` after this
migration. Do not run them while generated artifacts are dirty, and do not enable or
deploy a schedule as part of local validation.

---

## Track4 FoundryIQ 자동 검증 샘플 패키지

Track4 실습(에이전트 헬스체크 → 통합 응답 → fallback 검증)을 **키 입력 없이 로컬에서 재현**할 수 있도록 만든 샘플 패키지입니다.

### 1) 목적

- Track1 정형 데이터(CSV)와 Track2 비정형 콘텐츠 매니페스트를 결합해 Track4 질문(Q1~Q3) 실행 입력을 자동 생성
...
```bash
30 8 * * 1-5 cd /path/to/repo/track4/data && \
python generate_track3_samples.py && \
...
[track4/WORKBOOK.md](../WORKBOOK.md)의 미션 4(생성/평가 + 제출)에서 요구하는 **최종 브리핑**(요약 + 조치안 + 출처 링크,
...
| `AZURE_AI_FOUNDRY_BEARER_TOKEN` | ****** 인증 사용 시 토큰 (`Authorization` 헤더) | 선택 |
